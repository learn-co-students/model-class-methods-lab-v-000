class Captain < ActiveRecord::Base
  ...
  def self.sailors
    self.joins(:boats, :boat_classifications, :classifications).where(classifications: { :name => 'Sailboat' }).group('captains.id')
  end 

  def self.motorboaters
    self.joins(:boats, :boat_classifications, :classifications).where(classifications: { :name => 'Motorboat' }).group('captains.id')
  end 
